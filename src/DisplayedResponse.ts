class DisplayedResponse {

  
  // called all over but only in this file 
  static handleClickValid(this: HTMLElement, event:MouseEvent) { // only called by JS as handler
    console.log('DisplayedResponse.handleClickValid')
    SelectedEndpoints.addValid((event.target as HTMLElement).innerText)
  }

  // called all over but only in this file 
  static handleClickTemplate(this: HTMLElement, event:MouseEvent) {  // only called by JS as handler
    console.log('DisplayedResponse.handleClickTemplate')
    SelectedEndpoints.addTemplate((event.target as HTMLElement).innerText)  
  }


  static linkify(handleClickValid:ClickHander, handleClickTemplate:ClickHander) {
    console.log('DisplayedResponse.linkify')
    
    function runLinkify(handleClickValid:ClickHander, handleClickTemplate:ClickHander) {
      ...
      for (const element of span_str)
      {
        ...
        { // VALID URLs for GITHUB API ENDPOINT
          ...
          if (handleClickValid) {
            btn?.addEventListener("click", handleClickValid, false);
          }
        ...
        { // GITHUB API ENDPOINT URL TEMPLATES
          ...
          if (handleClickTemplate) {
            btn?.addEventListener("click", handleClickTemplate, false);
          }
