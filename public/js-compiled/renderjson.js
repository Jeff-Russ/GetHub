...
var renderjson = (function () {
    ...
    var A = function A(txt, classname, callback) {
        // creates link (i.e. <a class="disclosure" href="#">▼</a>)
    ...
    function _renderjson(json, indent, dont_indent, show_level, options) {
        ...
        var disclosure = function (open, placeholder, close, type, builder) {
            ...
            var show = function () {
                if (!content)
                    append(empty.parentNode, (content = prepend(builder(), A(options.hide, 'disclosure', function () {
        ...
        // || /^[0-9]+$/.test(Object.keys(json).join('')) // added by Jeff-Russ
        ) {
            if (json.length == 0)
            ...
            return disclosure('[', options.collapse_msg(json.length), ']', 'array', function () {
                ...
                for (var i = 0; i < json.length; i++)
                    append(as, _renderjson(options.replacer.call(json, i, json[i]), 
                    // indent + '    ',// removed by Jeff-Russ
                    indent + '  ', // added by Jeff-Russ
                    false, show_level - 1, options), i != json.length - 1 ? themetext('syntax', ',') : [], text('\n'));
        ...
        return disclosure('{', options.collapse_msg(Object.keys(json).length), '}', 'object', function () {
            ...
            for (var i in keys) {
                ...
                if (!(k in json))
                    continue;
                append(os, themetext(null, 
                // indent + '    ', // removed by Jeff-Russ
                indent + '  ', // added by Jeff-Russ
                'key', 
                // '"' + k + '"', // removed by Jeff-Russ
                k, // added by Jeff-Russ
                'object syntax', ': ' // removed by Jeff-Russ? nah
                ), _renderjson(options.replacer.call(json, k, json[k]), 
                // indent + '    ', // removed by Jeff-Russ
                indent + '  ', // added by Jeff-Russ
                true, show_level - 1, options), k != last ? themetext('syntax', ',') : [], text('\n'));
            }
    ...
    var renderjson = function renderjson(json) {
        // the actual renderjson function (returned from outer self-executing function)
